...
> **⚠️ 이 프로젝트는 현재 라이브 테스팅 및 개발 초기 단계(WIP)입니다.**
> 아키텍처 설계와 철학적 기반은 확립되었으나, 실제 런타임은 목표 아키텍처(PTC)로 진입하기 위한 8차 로드맵(Phase 4) 이행 중입니다. 브레이킹 체인지와 불완전한 통합이 자주 발생합니다.

### 📊 현재 상태 — 아키텍처 정렬도 스코어카드 (As of May 2026)

현재 런타임 코드가 목표 설계(Target Architecture)에 얼마나 도달했는지를 나타내는 지표입니다:

| 아키텍처 컴포넌트 | 정렬도 | 현재 상태 및 다음 과제 (Next Steps) |
|---|---|---|
| **영속적 지식 베이스 (Vault SOT)** | **100%** | 엄격한 마크다운 프론트매터 제약 및 생명주기(ACTIVE/SUPERSEDED) 전면 구현 완료. |
| **추론 자원 임대 (Reasoning Lease)** | **90%** | 프로바이더 간의 `source_ref` 포인터 전달 및 샌드박스 내부 추론 위임 기반 마련. |
| **소비/검색 파이프라인 (Retrieval)** | **60%** | 기존 거대 모놀리식 모듈에서 탈피하여 Tool-based Retrieval 체계로 전환 중. |
| **지형 기반 카테고리 (Map Maker)** | **40%** | 대륙(Continent) 분리 및 나이테(Tree Ring) 개념은 도입되었으나, 실제 물리적 식재(Gardener)와의 완벽한 라우팅 연결은 작업 중. |
| **생산 파이프라인 (PTC 엔진)** | **30%** | **(가장 큰 간극)** 현재는 결정론적(Deterministic) Python 체인(`thin_worker_chain.py`)으로 흉내 내고 있습니다. 서브에이전트가 직접 코드를 작성하여 생산 도구를 호출하는 **진정한 다면기(Role-Polymorphic Executor)로의 마이그레이션이 최우선 과제**입니다. |
...
### 생산 트리거 — 프로바이더 본체의 맥락 인지와 의뢰 (1차 구조화)

프로바이더 본체 에이전트(예: Hermes, Claude Code)는 사용자와 대화하며 **"이 아키텍처 결정이나 디버깅 맥락은 영구적인 지식(Wiki)으로 기록해야 한다"**는 니즈를 능동적으로 인지합니다. 

이때 본체 에이전트는 무거운 전체 텍스트를 복사하여 넘기지 않습니다. 대신 `SKILL.md`를 참고하여 **어디를 읽으면 되는지 가리키는 원본 포인터(jsonl)와 얕은 요약본**으로 구성된 `Session Structure Signal`을 생성해 서브에이전트에게 의뢰를 주입합니다.

```
  프로바이더 본체 에이전트 (사용자와 직접 대화하는 주체)
       │
       ...
       │     }
       │
       │  ④ 캡처 진입점 호출 및 Reasoning Lease(추론 자원) 위임
       │     → OpenYggdrasil 콜드스타트 및 서브에이전트 스폰
       │
       ▼
  OpenYggdrasil 런타임 (의뢰 수신 및 역할 가변 서브에이전트로 위임)
```

**핵심 규칙:**
- **포인터 기반 의뢰 (`source_ref` 필수):** 본체 에이전트는 원본 대화를 훼손하거나 복제하지 않습니다. 반드시 `turn_delta.v1.jsonl` 등의 로그 파일 위치를 가리키는 `source_ref` 포인터를 넘겨야 합니다. 이를 누락한 신호는 Admission Gate에서 거부됩니다.
...
2. **심층 구조화 (서브에이전트):** 런타임은 이 의뢰를 받아 프로바이더의 추론 자원(Reasoning Lease)을 빌려 서브에이전트를 스폰합니다. 서브에이전트는 고정된 파이프라인이나 단일 모듈이 아닌, **부여된 작업 계약(Task Contract)에 따라 역할을 바꾸는 다면기(Role-Polymorphic Leased Executor)**입니다.

서브에이전트는 PTC(Programmatic Tool Calling) 본질에 맞게, 부여된 생산 역할(Distiller, Evaluator, Amundsen, Gardener 등)을 수행하기 위해 **필요한 코드를 직접 작성하여 허용된 OpenYggdrasil의 도구를 호출**합니다. (8개 도구를 기계적으로 순차 호출하는 것이 아닙니다.)

이 체인을 구성하는 OpenYggdrasil의 생산/기록 도구들은 두 가지 성격으로 나뉩니다:

- **계약 가드레일 (추론 요구):** 서브에이전트의 추론 토큰을 소비하여 심층 의사결정(증류, 가치 평가, 분류 등)을 수행하도록 유도하되, 출력 형태를 엄격히 제약합니다.
- **작업 도구 (추론 불필요):** 서브에이전트가 가드레일을 통과한 결과물을 정규화, 기록, 포장할 수 있게 돕는 순수 Python 유틸리티입니다.

```
  Session Structure Signal (프로바이더 에이전트가 맥락을 인지하여 주입)
       │
       ▼
  PTC Subagent (지식 생산/기록을 담당하는 역할 가변 추론 실행체)
       │
       │  ① OpenYggdrasil이 Task Contract (Distiller/Amundsen/Gardener 등) 부여
       │  ② 서브에이전트가 부여된 역할에 맞춰 코드를 작성하여 생산 도구 호출
       │
       ▼
  ...
  └──────────────────────────────────────────────────────────┘
       │
       ▼
  서브에이전트가 수신증(Receipt)을 쥐고 프로바이더 세션으로 복귀 (생산 및 기록 완료)
```
