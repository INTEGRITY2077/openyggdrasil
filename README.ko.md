...
### 2. 카테고리 구조화 = 대륙의 정의 (Continents & Terrain)
openyggdrasil에서 카테고리를 구조화하는 것은 **대륙(Continent)을 정의하는 것**과 같습니다. 지식은 평면적인 폴더가 아니라 생태계 지형(Terrain)입니다.
- **Amundsen(탐험가)**은 새로 들어온 지식이 기존 대륙('기존 카테고리')에 속하는지, 완전히 새로운 '신대륙'인지 판별하여 경계를 긋습니다.
- **Map Maker(지도 제작자)**는 해당 대륙 내에서 지식의 위상과 좌표를 기획합니다.
- **Gardener(정원사)**는 지식이 잘못된 대륙에 심기지 않도록 생태계를 보호하며 물리적으로 식재합니다.

### 3. 나이테와 계통수 (Tree Rings & Lineage)
*"시간은 선형으로 흐르지만, 맥락은 선형으로 진화하지 않습니다."*
단순히 최신 지식만 검색하면 과거의 중요한 근원(Origin)은 텔로미어처럼 닳아 없어집니다. 이를 방지하기 위해 지식을 **진화의 나무(Evolution Tree)**로 취급합니다:
- **나이테(Ring) 각인:** 모든 묘목에는 기원(`provider_id`, `session_uid`, `timestamp`)이 물리적으로 각인됩니다.
- 뿌리(Root)와 줄기(Trunk)가 되는 기초 결정은 보존되고, 폐기된 가지(Branch)는 명시적으로 가지치기(`SUPERSEDED`)됩니다. 이를 통해 언제든 지식의 기원과 진화 과정을 추적할 수 있습니다.
...
서브에이전트는 PTC(Programmatic Tool Calling) 본질에 맞게, 부여된 역할(Distiller, Evaluator, Amundsen, Pathfinder 등)을 수행하기 위해 **필요한 코드를 직접 작성하여 허용된 OpenYggdrasil의 도구를 호출**합니다. (8개 도구를 기계적으로 순차 호출하는 것이 아닙니다.)

이 체인을 구성하는 OpenYggdrasil의 도구들은 두 가지 성격으로 나뉩니다:
...
```
  Session Structure Signal (프로바이더 에이전트가 맥락을 인지하여 주입)
       │
       ▼
  PTC Subagent (역할 가변 추론 임대 실행체)
       │
       │  ① OpenYggdrasil이 Task Contract (Distiller/Amundsen/등) 부여
       │  ② 서브에이전트가 부여된 역할에 맞춰 코드를 작성하여 도구 호출
       │
       ▼
