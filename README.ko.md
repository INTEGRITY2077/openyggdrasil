...
캡처 신호가 시스템에 들어오면, 이 신호를 단순히 자동화된 블랙박스에 넘기지 않습니다. **서브에이전트가 8개 도구를 순차적으로 직접 호출합니다.**

이 과정은 두 단계의 구조화를 거칩니다:
1. **1차 구조화 (프로바이더 에이전트):** 프로바이더 에이전트가 `SKILL.md`를 참고하여 `surface_reason`과 `source_ref`가 포함된 얕은 초기 신호(Session Structure Signal)를 구성해 진입점에 주입합니다.
2. **2차 심층 구조화 (서브에이전트):** PTC 엔진에 의해 스폰된 서브에이전트가 아래의 `distill_signal` 도구를 이용해 그 얕은 신호를 깊이 있는 의사결정(Rationale, Alternatives)으로 완벽하게 구조화(증류)합니다.

도구는 두 가지 성격으로 나뉩니다:

- **계약 가드레일** (3개) — 서브에이전트의 추론을 구조화된 출력으로 유도합니다. 서브에이전트가 추론 토큰을 소비하지만, 가드레일이 출력 형태와 판단 기준을 제약합니다.
- **작업 도구** (5개) — 서브에이전트가 가드레일을 통과한 결과물을 스트레스 없이 정규화·기록·포장할 수 있게 돕는 Python 유틸리티입니다. 추론 불필요 — 서브에이전트가 Python 스크립트를 호출하면 됩니다.

```
  Session Structure Signal (프로바이더 에이전트가 1차 요약하여 주입)
       │
       ▼
  서브에이전트 스폰 (openyggdrasil 런타임 내부)
       │
       │  ① PTC 엔진이 서브에이전트에게 8개 도구와 실행 계획 제시
       │  ② 서브에이전트가 코드를 조합하여 8개 도구를 순서대로 호출
       │
       ▼
  ...
  │                                                          │
  │  ┌─ distill_signal ─────────────────────────────────┐    │
  │  │  얕은 1차 요약본을 깊이 있는 의사결정 후보로 심층 증류 │    │
  │  │  추론 깊이: HIGH                                   │    │
  │  └──────────────────────────────────────────────────┘    │
  │                      ▼                                   │
  │  ┌─ evaluate_candidate ─────────────────────────────┐    │
