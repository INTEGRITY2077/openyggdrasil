...
def _validate_surface_unavailable_evidence(surface_evidence: Mapping[str, Any]) -> None:
    classification = surface_evidence.get("classification")
    contract = classification.get("contract") if isinstance(classification, Mapping) else {}
    schema_version = contract.get("schema_version") if isinstance(contract, Mapping) else ""
    if schema_version == "p0_official_hermes_gateway_contract.v1":
...
def build_hermes_real_session_usage_unavailable_proof_package(
    ...
    _validate_surface_unavailable_evidence(surface_unavailable_evidence)
...
def build_hermes_real_session_usage_unavailable_projection(
    *,
    ...
    _validate_surface_unavailable_evidence(surface_evidence)
