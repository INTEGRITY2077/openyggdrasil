...
### 2. 지형 기반 카테고리 구조화 (Forest & Terrain)
지식은 평면적인 목록이 아니라 **지형(Terrain)**입니다. 
- **Amundsen(탐험가)**은 새로 들어온 지식이 기존 카테고리('백엔드 숲')에 속하는지, 완전히 새로운 영역('신대륙')인지 판별합니다.
- **Map Maker(지도 제작자)**는 지식의 상대적 위상과 좌표를 기획합니다.
- **Gardener(정원사)**는 잘못된 숲에 나무가 심기지 않도록 생태계를 보호하며 물리적으로 식재합니다.
