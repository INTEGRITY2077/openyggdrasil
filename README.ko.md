...
이때 프로바이더 에이전트는 무거운 전체 텍스트를 복사하여 넘기지 않습니다. 대신 `SKILL.md`를 참고하여 **어디를 읽으면 되는지 가리키는 원본 포인터(jsonl)와 얕은 요약본**으로 구성된 `Session Structure Signal`을 생성해 이그드라실 에이전트에게 의뢰를 주입합니다.
...
```text
  🤖 프로바이더 에이전트 (사용자와 직접 대화하는 Front-stage 주체)
       ...
       │     → OpenYggdrasil Session Start 및 오퍼레이터 세션 스폰
       │
       ▼
  🌳 오퍼레이터 세션 (Mailbox를 통해 의뢰를 수신하고 백그라운드에서 동작하는 주체)
```
...
- **추론 자원 임대 (Reasoning Lease):** 프로바이더 에이전트는 신호를 넘길 때, 오퍼레이터 세션이 이 `jsonl` 원본을 파싱하고 심층 구조화(Distill/Evaluate)를 수행할 수 있도록 자신의 추론 자원을 반드시 빌려주어야(Lease) 합니다.
